#include "oled.h"
#include "delay.h"
uint16_t tempI1,tempI2,tempI3,tempI4,tempV1,tempV2,tempV3,tempV4;
...
const unsigned char F16X16_hanzi[] =
{
	"温度"
};
...
//	0x80,0x60,0x1F,0x00,0x80,0x93,0x92,0x92,0x92,0xFF,0x92,0x92,0x92,0x93,0x80,0x00},//厘
//	{0x00,0x40,0x42,0x44,0x58,0x40,0xC0,0xFF,0xC0,0x40,0x50,0x48,0x46,0x40,0x00,0x00,
//	0x20,0x20,0x10,0x08,0x04,0x13,0x00,0xFF,0x00,0x03,0x04,0x08,0x10,0x20,0x20,0x00},//米
//	{0x40,0x48,0x48,0x48,0xFF,0x48,0x48,0x42,0xA2,0x9E,0x82,0xA2,0xC2,0xBE,0x00,0x00,
//	0x80,0x60,0x1F,0x20,0x7F,0x44,0x44,0x40,0x4F,0x48,0x48,0x48,0x48,0x4F,0x40,0x00},//超
...
//	0x04,0x04,0x7C,0x03,0x80,0x60,0x1F,0x80,0x43,0x2C,0x10,0x28,0x46,0x81,0x80,0x00},//波
...
//	0x00,0x08,0x09,0x78,0x04,0x03,0x00,0x3F,0x40,0x40,0x42,0x44,0x43,0x40,0x78,0x00},//范
//	{0x00,0xFE,0x02,0x12,0x52,0x52,0x52,0xFE,0x52,0x52,0x52,0x12,0x02,0xFE,0x00,0x00,
//	0x00,0xFF,0x40,0x41,0x41,0x41,0x41,0x7F,0x41,0x49,0x51,0x4F,0x40,0xFF,0x00,0x00},//围
//	{0x00,0x3E,0x22,0xE2,0x22,0x3E,0x00,0xFE,0x22,0x22,0x22,0x22,0x22,0xE2,0x02,0x00,
//	0x20,0x3F,0x20,0x1F,0x11,0x11,0x00,0x7F,0x44,0x44,0x44,0x44,0x44,0x47,0x40,0x00},//距
...
const unsigned char F12X16[][24] =
{
    {0x40,0xA0,0x40,0x1F,0x20,0x40,0x40,0x40,0x20,0x70,0x01,0x00,0x00,0x00,0x00,0x00,
0x80,0x40,0x40,0x40,0x40,0x80,0x00,0x00}   // ℃

};
const unsigned char F12X16_hanzi[] =
{
	"℃"
};
...
void OLED_Init(void)//oled初始化        
{
  ...
  OLED_GPIO_InitStructure.Pin = OLED_SCL|OLED_SDA|OLED_RST|OLED_DC|OLED_CS;
	OLED_GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_LOW;
	OLED_GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
	OLED_GPIO_InitStructure.Pull = GPIO_NOPULL;
//	OLED_GPIO_InitStructure1.GPIO_Pin = OLED_SCL|OLED_SDA;
//	OLED_GPIO_InitStructure1.GPIO_Speed = GPIO_Speed_50MHz;
//	OLED_GPIO_InitStructure1.GPIO_Mode = GPIO_Mode_Out_PP;
	HAL_GPIO_Init(OLED_GPIO, &OLED_GPIO_InitStructure);
	HAL_GPIO_WritePin(GPIOB, OLED_CS, GPIO_PIN_RESET);
//	GPIO_Init(OLED_GPIO1, &OLED_GPIO_InitStructure1);
	// 1. 使能PWR时钟
...
//  delay_ms(100);	
  ...
  OLED_WR_Byte(0xAE,OLED_CMD); //关闭显示
	...
	OLED_WR_Byte(0xC0,OLED_CMD); //设置COM扫描方向;bit3:0,普通模式;1,重定义模式 COM[N-1]->COM0;N:驱动路数 //0xc0反   0xc8正常
...
//返回：无
//==============================================================  
void OLED_P6x8Str(uint8_t x,uint8_t y,uint8_t ch[])//小字符串
{
  uint8_t c=0,i=0,j=0;
  OLED_Set_Pos(x,y);      
  while (ch[j]!='\0')
...
//返回：无
//==============================================================  
void OLED_P8x16Str(uint8_t x,uint8_t y,uint8_t ch[])//大字符串
{
  uint8_t c=0,i=0,j=0;
...
void OLED_P16x12Str(uint8_t x, uint8_t y, const unsigned char ch[],int adder)//汉字
{
	char wm = 0, ii = 0;
	while (ch[ii] != '\0')
	{
...
void OLED_P16x16Str(uint8_t x, uint8_t y, const unsigned char ch[],int adder)//汉字
{
	char wm = 0, ii = 0;
	while (ch[ii] != '\0')
	{
...
void OLED_P16x16kongge(uint8_t x, uint8_t y)//空格
{
	char wm = 0;
	OLED_Set_Pos(x, y);
	for (wm = 0; wm < 16; wm++)
...
void OLED_Num1(unsigned char x,unsigned char y,unsigned char asc) 
{
    ...
    for(i=0;i<8;i++)
    {
       OLED_WrDat(F8x16[asc][i]);         
    }
		OLED_Set_Pos(x*8,y+1);
		 for(i=0;i<8;i++)
    {
       OLED_WrDat(F8x16[asc][i+8]);         
    }
}
